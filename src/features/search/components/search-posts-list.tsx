...
export const SearchPostsList = ({ query, sortType }: SearchPostsListProps) => {
  ...
  return (
    <div className="space-y-4">
      {posts.map((post) => (
        <Link key={post.id} to={paths.app.post.getHref(post.id)}>
          <div className="group rounded-md border-b bg-card transition-colors hover:border-blue-200 hover:bg-blue-50/50">
            <div className="flex gap-4 p-4">
               ...
               */}
                {post.content && (
                  <MDPreview value={post.content} maxLines={2} />
                )}
